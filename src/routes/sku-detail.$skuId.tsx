...
import { fmtDateEN as formatDate } from "@/lib/fmt";
...
function SkuDetailPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<"all" | "Processed" | "Shipped" | "Cancelled">(
    "all",
  );
  ...
  const found = useMemo(() => {
    for (const b of brands)
      for (const c of b.categories)
        for (const s of c.skus) if (s.id === skuId) return { sku: s, brand: b, category: c };
    return null;
  }, [brands, skuId]);
      ...
      <div className="space-y-5 animate-fade-in">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 stagger">
          {[
            ...
            {
              label: "Total Revenue",
              value: formatIDR(metrics.revenue),
              icon: DollarSign,
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <div className="stat-value text-[15px] font-semibold mt-0.5 truncate">
                  {m.value}
                </div>
              </div>
            </div>
          ...
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-0.5">
                ...
                {allRelatedTx.length !== filteredTx.length && (
                  <span className="ml-1 opacity-60">· {allRelatedTx.length} total</span>
                )}
              </div>
            </div>
          </div>
                    ...
                    {pagedTx.map((t) => {
                      ...
                      return (
                        <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right text-[13px] font-medium tabular-nums">
                            {formatIDR(line.qty * line.unitPrice)}
                          </td>
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[10px] font-medium ${STATUS_COLORS[t.status] ?? "border-gray-400 bg-gray-400 text-white"}`}
                            >
                              {t.status}
                            </span>
                          </td>
              ...
              {totalPages > 1 && (
                <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between text-[11px] text-muted-foreground">
                  <span>
                    ...
                    {Math.min(page * PAGE_SIZE, filteredTx.length)} dari {filteredTx.length}
                  </span>
                  <div className="flex items-center gap-1">
                    <button
                      type="button"
