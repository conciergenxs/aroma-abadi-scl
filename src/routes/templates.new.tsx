          ...
          <FormCard step={1} title="Settings" description="Identify the template and pick how it appears in your library.">
            <Field label="Template Name" required>
              <input
                ...
                className="w-full h-10 rounded-md border border-border bg-[oklch(0.97_0_0)] px-3 text-[13px] focus:outline-none focus:ring-1 focus:ring-primary/40"
              />
            </Field>
                ...
                {showNewGroupInput ? (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 h-8 rounded-md border border-border bg-[oklch(0.97_0_0)] px-3 text-[12px] focus:outline-none focus:ring-1 focus:ring-primary/40"
                    />
                    <button
            ...
            <Field label="Promo Code (Optional)">
              {(() => {
                const filteredPromos = PROMO_CODES_LIST.filter(
                  p => !promoSearch || p.code.toLowerCase().includes(promoSearch.toLowerCase()) || p.name.toLowerCase().includes(promoSearch.toLowerCase())
                );
                const selected = PROMO_CODES_LIST.find(p => p.id === selectedPromoId);
                return (
                  <div className="relative">
                    <button
                      type="button"
                      onClick={() => setPromoDropdownOpen(o => !o)}
                      className="w-full h-9 flex items-center gap-2 rounded-md border border-border bg-[oklch(0.97_0_0)] px-3 text-[13px] text-left focus:outline-none focus:ring-1 focus:ring-primary/40"
                    >
                      <Tag className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                      {selected ? (
                        <span className="flex-1 truncate"><span className="font-medium text-primary">{selected.code}</span> <span className="text-muted-foreground">— {selected.name}</span></span>
                      ) : (
                        <span className="flex-1 text-muted-foreground">No promo code attached</span>
                      )}
                      {selected && (
                        <span
                          role="button"
                          onClick={e => { e.stopPropagation(); setSelectedPromoId("none"); setPromoSearch(""); }}
                          className="text-muted-foreground hover:text-destructive"
                        ><XIcon className="h-3.5 w-3.5" /></span>
                      )}
                    </button>
                    {promoDropdownOpen && (
                      <div className="absolute z-30 mt-1 w-full rounded-md border border-border bg-card shadow-lg overflow-hidden">
                        ...
                        <div className="max-h-48 overflow-y-auto">
                          <button
                            type="button"
                            onClick={() => { setSelectedPromoId("none"); setPromoSearch(""); setPromoDropdownOpen(false); }}
                            className="w-full text-left px-3 py-2 text-[12px] text-muted-foreground hover:bg-muted"
                          >
                            No promo code
                          </button>
                          {filteredPromos.map(p => (
                            <button
                              key={p.id}
                              type="button"
                              onClick={() => { setSelectedPromoId(p.id); setPromoSearch(""); setPromoDropdownOpen(false); }}
                              className={`w-full text-left px-3 py-2 text-[12px] flex items-center gap-2 transition-colors ${selectedPromoId === p.id ? "bg-primary/10 text-foreground" : "hover:bg-muted text-foreground"}`}
                            >
                              <span className="font-mono font-medium text-primary text-[11px]">{p.code}</span>
                              <span className="text-muted-foreground truncate">{p.name}</span>
                              {selectedPromoId === p.id && <Check className="h-3 w-3 text-primary ml-auto shrink-0" />}
                            </button>
                          ))}
                          {filteredPromos.length === 0 && (
                            <div className="px-3 py-4 text-center text-[11px] text-muted-foreground">No codes match</div>
                          )}
                        </div>
                      </div>
                    )}
            ...
            <Field label="Header Type (Optional)">
              ...
              {headerType === "text" && (
                <input
                  ...
                  className="mt-2 w-full h-9 rounded-md border border-border bg-[oklch(0.97_0_0)] px-3 text-[13px] focus:outline-none focus:ring-1 focus:ring-primary/40"
                />
              )}
          ...
          <FormCard step={2} title="Content" description="Write the body, add variables and optional footer or buttons.">
            <Field label="Message Body" required hint="Use {{variable}} tokens — they get replaced when sending.">
              <textarea
                ...
                placeholder={`Hi {{name}}, …`}
                className="w-full min-h-[160px] rounded-md border border-border bg-[oklch(0.97_0_0)] p-3 text-[13px] leading-relaxed focus:outline-none focus:ring-1 focus:ring-primary/40 resize-y"
              />
              <div className="mt-2 flex flex-wrap items-center gap-1.5 relative">
                {/* Name — direct insert */}
                <button
                  onClick={() => insertVariable("{{name}}")}
                  className="inline-flex items-center gap-1 rounded-md border border-border bg-[oklch(0.97_0_0)] hover:bg-[oklch(0.97_0_0)] px-2 h-7 text-[11px] text-muted-foreground"
                >
            ...
            <Field label="Footer (Optional)" hint="Short closing line, e.g. 'Reply STOP to opt out'.">
              <input
                ...
                className="w-full h-9 rounded-md border border-border bg-[oklch(0.97_0_0)] px-3 text-[13px] focus:outline-none focus:ring-1 focus:ring-primary/40"
              />
            </Field>
              ...
              {buttonType === "cta" && (
                <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <input
                    ...
                    className="h-9 rounded-md border border-border bg-[oklch(0.97_0_0)] px-3 text-[13px] focus:outline-none focus:ring-1 focus:ring-primary/40"
                  />
                  <input
                    value={buttonUrl}
