...
function StatusBadge({ status }: { status: PromoStatus }) {
  if (status === "active")
    return (
      <span
        key={status}
        className="badge-animate inline-flex items-center gap-1 rounded-full border border-emerald-700 bg-emerald-600 px-2 py-0.5 text-[10px] font-medium text-white"
      >
  ...
  if (status === "expired")
    return (
      <span
        key={status}
        className="badge-animate inline-flex items-center gap-1 rounded-full border border-rose-700 bg-rose-600 px-2 py-0.5 text-[10px] font-medium text-white"
      >
  ...
  return (
    <span
      key={status}
      className="badge-animate inline-flex items-center gap-1 rounded-full border border-slate-400 bg-slate-500 px-2 py-0.5 text-[10px] font-medium text-white"
    >
  ...
  return (
    <>
      ...
      <FloatingMenu
        ...
        align="end"
        width={148}
        className="rounded-lg border border-border bg-card shadow-xl py-1 animate-scale-in origin-top-right"
      >
        <button
          type="button"
          onClick={(e) => {
...
function PromoCodesPage() {
  ...
  return (
    <AppShell
      title="Promo Codes"
      subtitle="Create and manage discount codes, rules, and redemptions."
    >
      {/* Toolbar */}
      ...
      <AlertDialog open={!!deletingPromo} onOpenChange={(open) => !open && setDeletingPromo(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {deletingPromo &&
                `"${deletingPromo.name}" (${deletingPromo.code}) will be permanently deleted. This cannot be undone.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
