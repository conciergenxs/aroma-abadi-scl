  ...
  head: () => ({
    meta: [
      { title: "Beauty Ambassadors — Aroma Abadi" },
      {
        name: "description",
        content:
          "Manage Aroma Abadi Beauty Ambassador data: profile, brand, store, and WhatsApp login credentials.",
      },
    ],
  }),
  component: BAPage,
...
function BAPage() {
  ...
  const allStores = useMemo(
    () => [...new Set(bas.map((b) => b.store).filter(Boolean))].sort(),
    [bas],
  );
  const allPosisi = useMemo(
    () => [...new Set(bas.map((b) => b.position).filter(Boolean))].sort(),
    [bas],
  );
  ...
  return (
    <AppShell
      title="Beauty Ambassadors"
      subtitle="Profiles and WhatsApp login accounts for all Beauty Ambassadors."
    >
      <SectionCard>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {hasFilters && (
              <button
                ...
                className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground border border-border rounded-md px-2 h-8 transition-colors duration-150"
              >
              ...
              {paginated.map((b) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Td>
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          title={shown ? "Hide" : "Show (requires your account password)"}
                        >
                          {shown ? (
                        ...
                        {shown && (
                          <button
                            onClick={() => {
                              navigator.clipboard.writeText(b.password);
                    ...
                    <Td>{b.position}</Td>
                    <Td>
                      {b.store} · <span className="text-muted-foreground">{b.city}</span>
                    </Td>
                    <Td>
                      <div className="flex items-center gap-1 justify-end">
                        <button
                          onClick={() => setEditing(b)}
                          className="rounded px-2 h-8 text-[14px] border border-border hover:bg-gray-50 transition-colors"
                        >
                          Edit
                        </button>
                        <button
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-border">
          <span className="text-xs text-muted-foreground">
            {filtered.length} Beauty Ambassador{filtered.length !== 1 ? "s" : ""} · Page {page} of{" "}
            {totalPages}
          </span>
          ...
          <div className="w-full max-w-sm bg-background border border-border rounded-xl overflow-hidden modal-content">
            <div className="p-5 border-b border-border">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Reset password for{" "}
                <span className="text-foreground font-medium">{resetConfirm.name}</span>? A new
      ...
      {(showCreate || editing) && (
        <BAForm
          ...
          onSubmit={(input, id) => {
            if (id) {
              baStore.update(id, input);
              toast.success("BA updated");
            } else {
              const created = baStore.add(input);
              toast.success(`BA added. Password: ${created.password}`);
            }
            setShowCreate(false);
            setEditing(null);
          }}
...
const filterSelectCls =
  "h-8 rounded-md border border-border bg-card/60 px-2 text-xs focus:outline-none focus:ring-1 focus:ring-primary/40";

function Th({ children }: { children: React.ReactNode }) {
  return (
    <th className="text-left font-medium px-3 py-2.5 text-xs uppercase tracking-wide">
      {children}
    </th>
  );
}
...
function BAForm({
  ...
  initial: BA | null;
  onClose: () => void;
  onSubmit: (input: Omit<BA, "id">, id?: string) => void;
}) {
  const { brands } = useSkuStore();
  ...
  const [password, setPassword] = useState(
    initial?.password || baStore.generatePassword(areaCoordinator, name),
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex modal-backdrop">
      ...
      <form
        onSubmit={submit}
        className="w-full max-w-md bg-background border-l border-border overflow-y-auto slide-in-right"
      >
        <div className="p-5 border-b border-border flex items-start justify-between">
          <div>
            <div className="inline-flex items-center gap-2 text-sm text-primary mb-1">
              <BadgeCheck className="h-3.5 w-3.5" /> Beauty Ambassador
            </div>
            <div className="text-base font-semibold">
              {initial ? "Edit Beauty Ambassador" : "Add New Beauty Ambassador"}
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="p-5 space-y-3">
          ...
          <Field label="Area Coordinator (ARCO)">
            <input
              ...
              onChange={(e) => setAreaCoordinator(e.target.value)}
              placeholder="Enter Area Coordinator name…"
              className={inputCls}
            />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Gender">
              <select
                value={gender}
                onChange={(e) => setGender(e.target.value as BA["gender"])}
                className={inputCls}
              >
                <option>Female</option>
                <option>Male</option>
          ...
          <Field label="Password (generated)">
            <div className="relative flex items-center">
              <input
                ...
                className={`${inputCls} font-mono bg-muted/40 cursor-not-allowed pr-16`}
              />
...
const inputCls =
  "h-9 w-full rounded-md border border-border bg-card/60 px-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-primary/40";
        ...
        <ul className="absolute z-30 top-10 left-0 right-0 rounded-md border border-border bg-background shadow-lg max-h-44 overflow-y-auto">
          ...
          {filtered.length === 0 && (
            <li className="px-3 py-2 text-sm text-muted-foreground">
              Not found — type to add manually
            </li>
          )}
        </ul>
      )}
