      ...
      <SectionCard>
        <div className="p-3 flex flex-wrap items-center gap-2 border-b border-border">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {hasFilters && (
              <button onClick={clearFilters} className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground border border-border rounded-md px-2 h-8 transition-colors duration-150">
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4 modal-backdrop">
          <div className="w-full max-w-sm bg-background border border-border rounded-xl overflow-hidden modal-content">
            ...
            <div className="p-4 flex justify-end gap-2">
              <button onClick={() => setResetConfirm(null)} className="rounded-md border border-border px-3 h-9 text-[14px]">Cancel</button>
              <button
                onClick={() => {
...
function RevealPasswordModal({ ba, onClose, onConfirmed }: { ba: BA; onClose: () => void; onConfirmed: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4 modal-backdrop">
      <form onSubmit={submit} className="w-full max-w-sm bg-background border border-border rounded-xl overflow-hidden modal-content">
        <div className="p-5 border-b border-border">
          <div className="inline-flex items-center gap-2 text-sm font-semibold"><Lock className="h-3.5 w-3.5 text-muted-foreground" /> Confirm Your Password</div>
          <p className="mt-1 text-sm text-muted-foreground">
            Enter your account password to view <span className="text-foreground font-medium">{ba.name}</span>'s login password.
          </p>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex modal-backdrop">
      ...
      <form onSubmit={submit} className="w-full max-w-md bg-background border-l border-border overflow-y-auto slide-in-right">
        <div className="p-5 border-b border-border flex items-start justify-between">
          ...
          <button type="button" onClick={onClose} className="text-muted-foreground hover:text-foreground transition-colors duration-150"><X className="h-4 w-4" /></button>
        </div>
        ...
        <div className="p-5 border-t border-border flex items-center justify-end gap-2">
          <button type="button" onClick={onClose} className="rounded-md border border-border px-3 h-9 text-[14px]">Cancel</button>
          <button type="submit" className="rounded-md bg-primary text-primary-foreground px-3 h-9 text-[14px] font-medium">{initial ? "Save" : "Add BA"}</button>
        </div>
      </form>
    </div>
  );
