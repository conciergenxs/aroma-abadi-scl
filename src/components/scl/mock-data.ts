export type Channel = "whatsapp";
export type LabelColor = "indigo" | "pink" | "emerald" | "amber" | "sky" | "violet" | "slate";

export const LIFECYCLE_STAGES = [
  "New Lead",
  "Contacted",
  "Qualified",
  "Pending Payment",
  "Customer",
  "Lost",
  "No Reply",
] as const;
export type LifecycleStage = string;

export const PROGRESSING_STAGES: string[] = [
  "New Lead",
  "Contacted",
  "Qualified",
  "Pending Payment",
  "Customer",
];
export const LOST_STAGES: string[] = ["Lost", "No Reply"];

export const STAGE_COLORS: Record<string, { bar: string; dot: string; badge: string }> = {
  "New Lead":        { bar: "bg-orange-500", dot: "bg-orange-500", badge: "border-orange-500/30 bg-orange-500/10 text-orange-300" },
  "Contacted":       { bar: "bg-blue-500",   dot: "bg-blue-500",   badge: "border-blue-500/30 bg-blue-500/10 text-blue-300" },
  "Qualified":       { bar: "bg-purple-500", dot: "bg-purple-500", badge: "border-purple-500/30 bg-purple-500/10 text-purple-300" },
  "Pending Payment": { bar: "bg-yellow-500", dot: "bg-yellow-500", badge: "border-yellow-500/30 bg-yellow-500/10 text-yellow-300" },
  "Customer":        { bar: "bg-green-500",  dot: "bg-green-500",  badge: "border-green-500/30 bg-green-500/10 text-green-300" },
  "Lost":            { bar: "bg-red-500",    dot: "bg-red-500",    badge: "border-red-500/30 bg-red-500/10 text-red-300" },
  "No Reply":        { bar: "bg-gray-500",   dot: "bg-gray-500",   badge: "border-gray-500/30 bg-gray-500/10 text-gray-300" },
};

export type ContactLabel = {
  id: string;
  name: string;
  color: LabelColor;
};

export type ContactList = {
  id: string;
  name: string;
  description?: string;
};

export type Contact = {
  id: string;
  name: string;
  phone: string;
  email?: string;
  instagram?: string;
  channel: Channel;
  labelIds: string[];
  listIds: string[];
  lastInteraction: string;
  status: "Active" | "Inactive" | "Blocked";
  avatar: string;
  ownerId?: string;
  lifecycleStage?: string;
  stageEnteredAt?: string;
  customFields?: Record<string, unknown>;
  deleted?: boolean;
  deletedAt?: string;
};

const avatars = ["PA", "BP", "SR", "DL", "RM", "IP", "AS", "MK", "CH", "RW", "BH", "NS", "AF", "YP", "TH", "LW", "GS", "NK", "FR", "ZA", "MR", "DP", "HS", "IA", "WS", "KN", "SB", "RP", "OL", "EM"];

export const initialLabels: ContactLabel[] = [
  { id: "lb-vip", name: "VIP Customer", color: "amber" },
  { id: "lb-beauty-club", name: "Beauty Club", color: "pink" },
  { id: "lb-reseller", name: "Reseller", color: "indigo" },
  { id: "lb-ba", name: "Brand Ambassador", color: "violet" },
  { id: "lb-team", name: "Internal Team", color: "slate" },
];

export const initialLists: ContactList[] = [
  { id: "ls-vip", name: "VIP Customers", description: "Top loyalty tier members" },
  { id: "ls-jakarta", name: "Customers — Jakarta", description: "Tunjungan Plaza & PIM stores" },
  { id: "ls-bandung", name: "Customers — Bandung", description: "Paris van Java & TSM stores" },
  { id: "ls-launch", name: "Product Launch Audience", description: "Opted-in to new SKU launches" },
  { id: "ls-community", name: "Aroma Beauty Club", description: "WhatsApp loyalty community" },
];

export const contacts: Contact[] = [
  { id: "c1", name: "Putri Anggraini", phone: "+62 812 3456 7890", channel: "whatsapp", labelIds: ["lb-vip", "lb-beauty-club"], listIds: ["ls-vip", "ls-launch"], lastInteraction: "2m ago", status: "Active", avatar: avatars[0], ownerId: "arma" },
  { id: "c2", name: "Bagus Pratama", phone: "+62 813 2244 5566", channel: "whatsapp", labelIds: ["lb-reseller"], listIds: ["ls-launch"], lastInteraction: "11m ago", status: "Active", avatar: avatars[1], ownerId: "arma" },
  { id: "c3", name: "Siti Rahmawati", phone: "+62 821 9988 7766", channel: "whatsapp", labelIds: ["lb-vip"], listIds: ["ls-vip", "ls-jakarta"], lastInteraction: "32m ago", status: "Active", avatar: avatars[2], ownerId: "arma" },
  { id: "c4", name: "Dewi Lestari", phone: "+62 811 5566 1122", channel: "whatsapp", labelIds: ["lb-beauty-club"], listIds: ["ls-bandung"], lastInteraction: "1h ago", status: "Active", avatar: avatars[3] },
  { id: "c5", name: "Rizky Maulana", phone: "+62 822 3344 7788", channel: "whatsapp", labelIds: [], listIds: ["ls-community"], lastInteraction: "2h ago", status: "Active", avatar: avatars[4], ownerId: "arma" },
  { id: "c6", name: "Indah Permata", phone: "+62 815 7788 9900", channel: "whatsapp", labelIds: ["lb-vip"], listIds: ["ls-vip", "ls-community"], lastInteraction: "3h ago", status: "Active", avatar: avatars[5], ownerId: "arma" },
  { id: "c7", name: "Andika Saputra", phone: "+62 818 4422 1133", channel: "whatsapp", labelIds: ["lb-reseller"], listIds: ["ls-jakarta"], lastInteraction: "5h ago", status: "Active", avatar: avatars[6], ownerId: "arma" },
  { id: "c8", name: "Maya Kusuma", phone: "+62 819 6655 2244", channel: "whatsapp", labelIds: ["lb-team"], listIds: [], lastInteraction: "Yesterday", status: "Inactive", avatar: avatars[7] },
  { id: "c9", name: "Citra Halim", phone: "+62 856 1122 3344", channel: "whatsapp", labelIds: [], listIds: ["ls-launch", "ls-community"], lastInteraction: "Yesterday", status: "Active", avatar: avatars[8], ownerId: "arma" },
  { id: "c10", name: "Reza Wijaya", phone: "+62 857 8899 0011", channel: "whatsapp", labelIds: ["lb-ba"], listIds: [], lastInteraction: "2d ago", status: "Active", avatar: avatars[9], ownerId: "arma" },
  { id: "c11", name: "Bayu Hartanto", phone: "+62 858 5544 3322", channel: "whatsapp", labelIds: ["lb-vip", "lb-reseller"], listIds: ["ls-vip", "ls-jakarta"], lastInteraction: "3d ago", status: "Active", avatar: avatars[10], ownerId: "arma" },
  { id: "c12", name: "Nadya Salsabila", phone: "+62 877 2211 4455", channel: "whatsapp", labelIds: ["lb-beauty-club"], listIds: ["ls-bandung", "ls-launch"], lastInteraction: "4d ago", status: "Active", avatar: avatars[11] },

  // ── Customer Contacts ──────────────────────────────────────────────
  { id: "c13", name: "Ayu Fitriani", phone: "+62 812 8877 6655", channel: "whatsapp", labelIds: ["lb-vip"], listIds: ["ls-vip", "ls-jakarta"], lastInteraction: "1h ago", status: "Active", avatar: avatars[12], ownerId: "arma" },
  { id: "c14", name: "Yusuf Priyanto", phone: "+62 821 3322 9988", channel: "whatsapp", labelIds: ["lb-reseller"], listIds: ["ls-launch"], lastInteraction: "3h ago", status: "Active", avatar: avatars[13], ownerId: "arma" },
  { id: "c15", name: "Tiara Hapsari", phone: "+62 857 4411 0022", channel: "whatsapp", labelIds: ["lb-beauty-club"], listIds: ["ls-community", "ls-launch"], lastInteraction: "5h ago", status: "Active", avatar: avatars[14], ownerId: "arma" },
  { id: "c16", name: "Lina Wulandari", phone: "+62 813 6655 4433", channel: "whatsapp", labelIds: ["lb-vip", "lb-beauty-club"], listIds: ["ls-vip", "ls-community"], lastInteraction: "Yesterday", status: "Active", avatar: avatars[15], ownerId: "arma" },
  { id: "c17", name: "Galih Santoso", phone: "+62 878 9900 1122", channel: "whatsapp", labelIds: [], listIds: ["ls-jakarta"], lastInteraction: "Yesterday", status: "Active", avatar: avatars[16], ownerId: "arma" },
  { id: "c18", name: "Nita Kurniasari", phone: "+62 822 7788 5566", channel: "whatsapp", labelIds: ["lb-beauty-club"], listIds: ["ls-bandung", "ls-community"], lastInteraction: "2d ago", status: "Active", avatar: avatars[17], ownerId: "arma" },
  { id: "c19", name: "Fajar Ramadhan", phone: "+62 819 5544 3311", channel: "whatsapp", labelIds: ["lb-reseller"], listIds: ["ls-launch", "ls-jakarta"], lastInteraction: "2d ago", status: "Active", avatar: avatars[18], ownerId: "arma" },
  { id: "c20", name: "Zahra Aulia", phone: "+62 856 0011 2233", channel: "whatsapp", labelIds: ["lb-vip"], listIds: ["ls-vip"], lastInteraction: "3d ago", status: "Active", avatar: avatars[19], ownerId: "arma" },
  { id: "c21", name: "Mira Rahmawati", phone: "+62 815 4455 8899", channel: "whatsapp", labelIds: [], listIds: ["ls-community"], lastInteraction: "3d ago", status: "Active", avatar: avatars[20], ownerId: "arma" },
  { id: "c22", name: "Dian Puspita", phone: "+62 877 6677 3344", channel: "whatsapp", labelIds: ["lb-beauty-club"], listIds: ["ls-bandung"], lastInteraction: "4d ago", status: "Active", avatar: avatars[21], ownerId: "arma" },

  // ── BA Contacts ────────────────────────────────────────────────────
  { id: "c23", name: "Hesti Andriani", phone: "+62 811 2233 4455", channel: "whatsapp", labelIds: ["lb-ba"], listIds: [], lastInteraction: "30m ago", status: "Active", avatar: avatars[22], ownerId: "arma" },
  { id: "c24", name: "Indra Wahyudi", phone: "+62 821 8877 6655", channel: "whatsapp", labelIds: ["lb-ba"], listIds: [], lastInteraction: "1h ago", status: "Active", avatar: avatars[23], ownerId: "arma" },
  { id: "c25", name: "Wulan Sari", phone: "+62 857 3344 5566", channel: "whatsapp", labelIds: ["lb-ba"], listIds: [], lastInteraction: "2h ago", status: "Active", avatar: avatars[24], ownerId: "arma" },
  { id: "c26", name: "Kevin Nugroho", phone: "+62 812 9900 8877", channel: "whatsapp", labelIds: ["lb-ba"], listIds: [], lastInteraction: "4h ago", status: "Active", avatar: avatars[25], ownerId: "arma" },
  { id: "c27", name: "Sinta Bella", phone: "+62 819 1122 7788", channel: "whatsapp", labelIds: ["lb-ba"], listIds: [], lastInteraction: "Yesterday", status: "Active", avatar: avatars[26], ownerId: "arma" },
  { id: "c28", name: "Rizal Pratama", phone: "+62 878 5566 4433", channel: "whatsapp", labelIds: ["lb-ba"], listIds: [], lastInteraction: "Yesterday", status: "Active", avatar: avatars[27], ownerId: "arma" },
  { id: "c29", name: "Olivia Lim", phone: "+62 813 3344 9900", channel: "whatsapp", labelIds: ["lb-ba"], listIds: [], lastInteraction: "2d ago", status: "Active", avatar: avatars[28], ownerId: "arma" },
  { id: "c30", name: "Eka Marlinda", phone: "+62 856 7788 1122", channel: "whatsapp", labelIds: ["lb-ba"], listIds: [], lastInteraction: "2d ago", status: "Active", avatar: avatars[29], ownerId: "arma" },
];

export type Message = {
  id: string;
  from: "them" | "me";
  text: string;
  time: string;
  status?: "sent" | "delivered" | "read";
};

export const threadsByContact: Record<string, Message[]> = {
  c1: [
    { id: "m1", from: "them", text: "Halo kak! Lipstick Velvet Rouge shade 03 ready stock ga?", time: "10:22" },
    { id: "m2", from: "me", text: "Halo Putri! Shade 03 baru restock pagi ini. Mau aku reserve satu?", time: "10:23", status: "read" },
    { id: "m3", from: "them", text: "Mau dong 🙏 boleh pakai diskon Beauty Club ku?", time: "10:24" },
    { id: "m4", from: "me", text: "Sudah diaplikasikan. Link checkout privat: aroma.id/v/putri-22", time: "10:25", status: "read" },
    { id: "m5", from: "them", text: "Makasih banyak! Aku checkout sekarang ya.", time: "10:26" },
  ],
  c2: [
    { id: "m1", from: "them", text: "Min, untuk reseller Glow Serum minimum order berapa pcs?", time: "09:48" },
    { id: "m2", from: "me", text: "Halo Bagus! Mulai 24 pcs untuk harga reseller. Aku kirim katalognya ya.", time: "09:50", status: "delivered" },
  ],
  c3: [
    { id: "m1", from: "them", text: "Mau pesan hampers Lebaran 120 pax untuk karyawan kami.", time: "Yesterday" },
    { id: "m2", from: "me", text: "Siap! Aku kirim katalog corporate + harga bulk sekarang.", time: "Yesterday", status: "read" },
    { id: "m3", from: "them", text: "Oke, aku loop in tim procurement kami.", time: "Yesterday" },
  ],
};

export type Conversation = {
  id: string;
  contactId: string;
  channel: Channel;
  preview: string;
  time: string;
  unread: number;
  pinned?: boolean;
};

export const conversations: Conversation[] = [
  { id: "t1", contactId: "c1", channel: "whatsapp", preview: "Makasih banyak! Aku checkout sekarang ya.", time: "10:26", unread: 2, pinned: true },
  { id: "t2", contactId: "c2", channel: "whatsapp", preview: "Min, untuk reseller Glow Serum minimum order…", time: "09:48", unread: 1 },
  { id: "t3", contactId: "c3", channel: "whatsapp", preview: "Oke, aku loop in tim procurement kami.", time: "Yesterday", unread: 0 },
  { id: "t4", contactId: "c4", channel: "whatsapp", preview: "Suka banget shade barunya! Restock dong kak.", time: "Yesterday", unread: 0 },
  { id: "t5", contactId: "c5", channel: "whatsapp", preview: "Paket order #AA-82201 sudah sampai, makasih!", time: "2d", unread: 0 },
  { id: "t6", contactId: "c6", channel: "whatsapp", preview: "Ada diskon bundle Glow Set ga?", time: "2d", unread: 3 },
  { id: "t7", contactId: "c7", channel: "whatsapp", preview: "Besok aku kirim PO yang sudah ditandatangani.", time: "3d", unread: 0 },
  { id: "t8", contactId: "c8", channel: "whatsapp", preview: "Velvet Rouge shade 05 ready di Bandung?", time: "4d", unread: 0 },
];

export type Template = {
  id: string;
  name: string;
  category: "Marketing" | "Utility" | "Service" | "Reminder";
  channel: Channel;
  status: "Approved" | "Pending" | "Rejected" | "Draft";
  updated: string;
  body: string;
  groupId?: string;
  language?: string;
  rejectionReason?: string;
};

export type TemplateGroup = {
  id: string;
  name: string;
  color: "indigo" | "pink" | "emerald" | "amber" | "sky" | "violet" | "slate" | "rose";
};

export const initialTemplateGroups: TemplateGroup[] = [
  { id: "tg-promotions", name: "Promotions", color: "pink" },
  { id: "tg-loyalty", name: "Beauty Club", color: "amber" },
  { id: "tg-cs", name: "Customer Service", color: "sky" },
  { id: "tg-events", name: "Events", color: "violet" },
  { id: "tg-launches", name: "SKU Launches", color: "indigo" },
  { id: "tg-leadnurture", name: "Lead Nurturing", color: "emerald" },
  { id: "tg-transactional", name: "Transactional", color: "slate" },
];

export const TEMPLATE_LANGUAGES = [
  { code: "id_ID", name: "Indonesian" },
  { code: "en_US", name: "English (US)" },
  { code: "en_GB", name: "English (UK)" },
] as const;

export const templates: Template[] = [
  { id: "tp1", name: "Velvet Rouge Launch", category: "Marketing", channel: "whatsapp", status: "Approved", updated: "2h ago", body: "Hi {{1}}, koleksi Velvet Rouge baru saja launching! VIP Beauty Club dapat akses 24 jam lebih awal — tap untuk belanja.", groupId: "tg-promotions", language: "id_ID" },
  { id: "tp2", name: "Order Dikirim", category: "Utility", channel: "whatsapp", status: "Approved", updated: "1d ago", body: "Order kamu {{1}} sudah dikirim. Lacak di sini: {{2}}", groupId: "tg-transactional", language: "id_ID" },
  { id: "tp3", name: "Cart Reminder", category: "Marketing", channel: "whatsapp", status: "Approved", updated: "3d ago", body: "Hi {{1}}, cart kamu masih nunggu. Kami hold {{2}} untuk 24 jam ke depan.", groupId: "tg-promotions", language: "id_ID" },
  { id: "tp4", name: "Reminder Beauty Class", category: "Reminder", channel: "whatsapp", status: "Approved", updated: "1w ago", body: "Reminder: kelas makeup kamu besok jam {{1}} bersama {{2}}.", groupId: "tg-events", language: "id_ID" },
  { id: "tp5", name: "Follow-up Tiket", category: "Service", channel: "whatsapp", status: "Pending", updated: "5h ago", body: "Hi {{1}}, kami follow-up tiket #{{2}}. Sudah teratasi?", groupId: "tg-cs", language: "id_ID" },
  { id: "tp6", name: "Beauty Club Upgrade", category: "Marketing", channel: "whatsapp", status: "Draft", updated: "30m ago", body: "Selamat {{1}}! Kamu naik ke tier Platinum. Berikut benefit baru untukmu.", groupId: "tg-loyalty", language: "id_ID" },
  { id: "tp7", name: "Restock Alert", category: "Utility", channel: "whatsapp", status: "Approved", updated: "2w ago", body: "{{1}} yang kamu wishlist sudah ready stock lagi. Tap untuk order.", groupId: "tg-transactional", language: "id_ID" },
  { id: "tp8", name: "Pembayaran Gagal", category: "Service", channel: "whatsapp", status: "Pending", updated: "6h ago", body: "Halo {{1}} — pembayaran order {{2}} gagal. Coba lagi di sini.", groupId: "tg-cs", language: "id_ID" },
  { id: "tp9", name: "Flash Sale", category: "Marketing", channel: "whatsapp", status: "Rejected", updated: "1d ago", body: "🔥 FLASH SALE {{1}}! DISKON 80% SEMUA PRODUK — KLIK SEKARANG!", groupId: "tg-promotions", language: "id_ID", rejectionReason: "Konten menggunakan all-caps dan promosi berlebihan yang melanggar WhatsApp Business policy." },
];

export type Broadcast = {
  id: string;
  name: string;
  channel: Channel;
  audience: string;
  reach: number;
  delivered: number;
  read: number;
  clicks: number;
  sentAt: string;
  status: "Sent" | "Scheduled" | "Draft";
  channelId?: string;
  listIds?: string[];
  totalAudience?: number;
  sendMode?: "now" | "schedule";
  scheduleDate?: string;
  scheduleTime?: string;
  createdBy?: string;
  createdAt?: string;
  contentMode?: "template" | "manual";
  templateId?: string;
  body?: string;
  footer?: string;
  buttons?: { label: string; kind: "quick_reply" | "url" | "phone" }[];
  replied?: number;
  failed?: number;
};

export const broadcasts: Broadcast[] = [
  { id: "b1", name: "Velvet Rouge — VIP Early Access", channel: "whatsapp", audience: "VIP · 12,408", reach: 12408, delivered: 12380, read: 11244, clicks: 4128, sentAt: "Hari ini · 09:00", status: "Sent",
    channelId: "wa-main", listIds: ["ls-vip", "ls-launch"], totalAudience: 12408, sendMode: "now", createdBy: "Putri Anggraini", createdAt: "Hari ini · 08:42",
    contentMode: "template", templateId: "tp1", body: "Hi {{name}}, koleksi Velvet Rouge baru saja launching! VIP dapat akses 24 jam lebih awal.", footer: "Balas STOP untuk berhenti.",
    buttons: [{ label: "Belanja sekarang", kind: "url" }], replied: 824, failed: 28 },
  { id: "b2", name: "Glow Serum — Reseller Promo", channel: "whatsapp", audience: "Reseller · 4,910", reach: 4910, delivered: 4880, read: 4112, clicks: 1342, sentAt: "Kemarin", status: "Sent",
    channelId: "wa-bdg", listIds: ["ls-launch"], totalAudience: 4910, sendMode: "now", createdBy: "Bagus Pratama", createdAt: "Kemarin · 17:10",
    contentMode: "template", templateId: "tp3", body: "Hi {{name}}, harga reseller Glow Serum bulan ini turun. Order minimum 24 pcs.", footer: "Aroma Abadi",
    buttons: [{ label: "Lihat katalog", kind: "url" }], replied: 312, failed: 30 },
  { id: "b3", name: "Promo Ramadhan Glow Set", channel: "whatsapp", audience: "Semua Kontak · 84,221", reach: 84221, delivered: 0, read: 0, clicks: 0, sentAt: "Jum · 08:00", status: "Scheduled",
    channelId: "wa-main", listIds: ["ls-community", "ls-launch"], totalAudience: 84221, sendMode: "schedule", scheduleDate: "2026-06-26", scheduleTime: "08:00",
    createdBy: "Putri Anggraini", createdAt: "2 hari lalu",
    contentMode: "manual", body: "Hi {{name}} — Promo Ramadhan Glow Set mulai Jumat jam 8 pagi. Akses awal untuk Beauty Club.", footer: "Balas STOP untuk berhenti.",
    buttons: [{ label: "Preview drop", kind: "url" }], replied: 0, failed: 0 },
  { id: "b4", name: "Corporate Hampers Lebaran", channel: "whatsapp", audience: "Corporate · 612", reach: 612, delivered: 610, read: 588, clicks: 244, sentAt: "3 hari lalu", status: "Sent",
    channelId: "wa-jkt", listIds: ["ls-vip"], totalAudience: 612, sendMode: "now", createdBy: "Siti Rahmawati", createdAt: "3 hari lalu",
    contentMode: "template", templateId: "tp2", body: "Hi {{name}}, katalog hampers Lebaran corporate kamu sudah siap. Harga bulk terlampir.",
    buttons: [{ label: "Download katalog", kind: "url" }], replied: 48, failed: 2 },
  { id: "b5", name: "Beauty Club Tier Refresh", channel: "whatsapp", audience: "Pelanggan Aktif · 22,114", reach: 22114, delivered: 0, read: 0, clicks: 0, sentAt: "—", status: "Draft",
    channelId: "wa-main", listIds: ["ls-community"], totalAudience: 22114, sendMode: "now", createdBy: "Indah Permata", createdAt: "30 menit lalu",
    contentMode: "template", templateId: "tp6", body: "Selamat {{name}}! Kamu naik ke tier Beauty Club berikutnya. Berikut benefit baru untukmu.",
    buttons: [{ label: "Lihat benefit", kind: "url" }], replied: 0, failed: 0 },
];

// Charts (WhatsApp-only)
export const volumeSeries = [
  { d: "Sen", whatsapp: 4200 },
  { d: "Sel", whatsapp: 5120 },
  { d: "Rab", whatsapp: 4880 },
  { d: "Kam", whatsapp: 6020 },
  { d: "Jum", whatsapp: 7240 },
  { d: "Sab", whatsapp: 5410 },
  { d: "Min", whatsapp: 4980 },
];

export const channelPerf = [
  { name: "WhatsApp", value: 100 },
];

export const contactGrowth = [
  { m: "Jun", v: 12400 },
  { m: "Jul", v: 14820 },
  { m: "Agu", v: 16210 },
  { m: "Sep", v: 18540 },
  { m: "Okt", v: 21810 },
  { m: "Nov", v: 26120 },
  { m: "Des", v: 29840 },
];

export const recentActivity = [
  { id: "a1", icon: "send", text: "Broadcast Velvet Rouge — VIP Early Access terkirim ke 12,380 kontak", time: "2m lalu" },
  { id: "a2", icon: "user", text: "Siti Rahmawati ditandai VIP oleh Putri Anggraini", time: "14m lalu" },
  { id: "a3", icon: "msg", text: "Template Beauty Club Upgrade disubmit untuk review", time: "32m lalu" },
  { id: "a4", icon: "alert", text: "WhatsApp channel quality rating naik ke High", time: "1h lalu" },
  { id: "a5", icon: "user", text: "47 kontak baru di-import dari sinkronisasi POS", time: "2h lalu" },
];

export type ConnectedChannel = {
  id: string;
  channel: Channel;
  name: string;
  handle: string;
  status: "connected" | "disconnected";
};

export const connectedChannels: ConnectedChannel[] = [
  { id: "wa-main", channel: "whatsapp", name: "Aroma Abadi HQ", handle: "+62 811 9001 2233", status: "connected" },
  { id: "wa-jkt", channel: "whatsapp", name: "Aroma Abadi Jakarta", handle: "+62 811 9001 4455", status: "connected" },
  { id: "wa-bdg", channel: "whatsapp", name: "Aroma Abadi Bandung", handle: "+62 811 9001 6677", status: "connected" },
  { id: "wa-loyalty", channel: "whatsapp", name: "Aroma Beauty Club", handle: "+62 811 9001 8899", status: "connected" },
];
