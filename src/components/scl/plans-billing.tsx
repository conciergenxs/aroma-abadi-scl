...
function SubscriptionPage({
  ...
  const contactOption =
    contactOptions.find((option) => option.value === contactPackage) ?? contactOptions[0];

  return (
    <div className="space-y-5">
      <PageHeader
        title="Subscription"
        description="Manage billing, usage, add-ons, and invoices."
      />
          ...
          <div className="flex flex-col justify-between gap-4 rounded-lg border border-border bg-background/45 p-4">
            <div>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Includes subscription and active recurring add-ons.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="inline-flex h-9 items-center justify-center gap-2 rounded-md bg-primary px-3 text-[14px] font-medium text-primary-foreground hover:bg-primary/90 transition-colors duration-150"
              >
                Manage Plan
              </button>
              <button
                ...
                className="inline-flex h-9 items-center justify-center gap-2 rounded-md border border-border bg-card/60 px-3 text-[14px] font-medium hover:bg-card transition-colors duration-150"
              >
          ...
          <LimitCard
            ...
            onAdd={
              item.addon === "users"
                ? () =>
                    setPendingPurchase({
                      type: "Additional User Accounts",
                      quantity: `${userSeats} user`,
                      monthlyCost: userSeats * 149000,
                    })
                : item.addon === "contacts"
                  ? () =>
                      setPendingPurchase({
                        type: "Additional Contacts",
                        quantity: contactOption.label,
                        monthlyCost: contactOption.price,
                      })
                  : undefined
            }
          />
      ...
      <SectionTitle
        title="Add-ons"
        description="Increase workspace limits without changing your subscription plan."
      />
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        >
          ...
          <AddOnActions
            onCancel={() => setUserSeats(1)}
            onPurchase={() =>
              setPendingPurchase({
                type: "Additional User Accounts",
                quantity: `${userSeats} user${userSeats > 1 ? "s" : ""}`,
                monthlyCost: userSeats * 149000,
              })
            }
          />
        ...
        >
          ...
          <AddOnActions
            onCancel={() => setContactPackage("5000")}
            onPurchase={() =>
              setPendingPurchase({
                type: "Additional Contacts",
                quantity: contactOption.label,
...
function ManagePlanPage({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex rounded-md border border-border bg-card/60 p-1">
          {(["monthly", "yearly"] as BillingCycle[]).map((value) => (
            <button
              ...
              className={`h-8 rounded px-3 text-xs font-medium transition ${cycle === value ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
      ...
      <PageHeader
        title="Manage Plan"
        description="Compare plans and choose the best fit for your workspace."
      />
          ...
          <PlanCard
            ...
            onSelect={() =>
              plan.name === "Enterprise"
                ? toast.info("Sales request prepared")
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        <PageHeader
          ...
          description="Manage saved cards and default billing method."
        />
        <button
          type="button"
          onClick={openNew}
          className="inline-flex h-9 items-center gap-2 rounded-md bg-primary px-3 text-[14px] font-medium text-primary-foreground hover:bg-primary/90 transition-colors duration-150"
        >
      ...
      <SectionCard title="Other Payment Methods">
        <div className="divide-y divide-border">
          ...
          {otherMethods.length === 0 && (
            <EmptyState
              title="No other payment methods"
              description="Additional cards will appear here."
            />
          )}
        </div>
      </SectionCard>
          ...
          setMethods((current) => {
            if (editing)
              return current.map((item) =>
                item.id === editing.id ? { ...item, ...method } : item,
              );
            return [
              ...current,
              { ...method, id: `pm-${Date.now()}`, isDefault: current.length === 0 },
            ];
          });
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          setMethods((current) =>
            current.map((method) => ({ ...method, isDefault: method.id === defaultTarget.id })),
          );
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="m-0 mt-1 text-xs text-muted-foreground">
              {fmtNum(current)} / {fmtNum(max)} {unit} used
            </p>
          </div>
...
function LimitCard({
  ...
  const percent =
    unlimited || !current || !max ? null : Math.min(100, Math.round((current / max) * 100));
  return (
    <SectionCard>
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          {onAdd && (
            <button
              type="button"
              onClick={onAdd}
              className="inline-flex h-8 items-center gap-1.5 rounded-md border border-border bg-card/60 px-2.5 text-xs font-medium hover:bg-card transition-colors duration-150"
            >
        ...
        {percent === null ? (
          <div className="mt-4 rounded-full border border-border bg-background/50 px-3 py-2 text-xs text-muted-foreground">
  ...
  return (
    <div className="inline-flex items-center overflow-hidden rounded-md border border-border bg-card/70">
      <button
        ...
        className="grid h-8 w-8 place-items-center hover:bg-gray-50 transition-colors duration-150"
        aria-label="Decrease quantity"
      >
        <Minus className="h-3.5 w-3.5" />
      </button>
      <div className="grid h-8 w-10 place-items-center border-x border-border text-sm font-medium">
        {value}
      </div>
      <button
        type="button"
...
function AddOnTotal({ amount }: { amount: number }) {
  return (
    <div className="flex items-center justify-between rounded-lg border border-border bg-background/45 p-3 text-sm">
      <span className="text-muted-foreground">Live total</span>
      <span className="font-semibold">{formatIdr(amount)} / month</span>
    </div>
  );
}
  ...
  return (
    <div className="flex justify-end gap-2">
      <button
        type="button"
        onClick={onCancel}
        className="h-9 rounded-md border border-border bg-card/60 px-3 text-[14px] font-medium hover:bg-card transition-colors duration-150"
      >
        Cancel
      </button>
      <button
        type="button"
        onClick={onPurchase}
        className="h-9 rounded-md bg-primary px-3 text-[14px] font-medium text-primary-foreground hover:bg-primary/90 transition-colors duration-150"
      >
        Purchase Add-on
      </button>
    </div>
...
function PurchaseConfirmDialog({
  ...
  return (
    <ConfirmDialog
      ...
      description={
        purchase
          ? `Add-on Type: ${purchase.type}\nQuantity: ${purchase.quantity}\nMonthly Cost: ${formatIdr(purchase.monthlyCost)}\nNew Monthly Total: ${formatIdr(currentMonthlyTotal + purchase.monthlyCost)}`
          : undefined
      }
          ...
          <div className="flex items-center gap-1 border-b border-border">
            {(["subscription", "addons"] as InvoiceTab[]).map((item) => (
              <button
                ...
                type="button"
                onClick={() => setTab(item)}
                className={`relative px-3 py-2 text-sm transition ${tab === item ? "text-foreground" : "text-muted-foreground hover:text-foreground"}`}
              >
            ...
            >
              ...
              <td className="px-5 py-3 text-right">
                <button
                  ...
                  className="inline-grid h-8 w-8 place-items-center rounded-md border border-border bg-card/60 hover:bg-card transition-colors duration-150"
  ...
  return (
    <SectionCard className={plan.badge ? "border-primary/40" : ""}>
      <div className="p-5 flex h-full min-h-[360px] flex-col">
        ...
        {cycle === "yearly" && plan.numericPrice > 0 && (
          <div className="mt-1 text-xs text-primary">14% saved yearly</div>
        )}
        ...
        <button
          ...
          disabled={disabled}
          onClick={onSelect}
          className={`mt-6 h-9 rounded-md px-3 text-[14px] font-medium ${disabled ? "cursor-not-allowed border border-border bg-muted text-muted-foreground" : "bg-primary text-primary-foreground hover:bg-primary/90"}`}
        >
          {plan.button}
        </button>
      </div>
        ...
        <table className="w-full min-w-[760px] text-sm">
          <thead className="border-b border-border bg-muted/20 text-xs text-muted-foreground">
            <tr>
              ...
              {plans.map((plan) => (
                <th key={plan.name} className="px-5 py-3 text-left font-medium">
                  {plan.name}
                </th>
              ))}
            </tr>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[280px_1fr_auto] lg:items-center">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <SummaryMetric label="Card Holder" value={method.holder} />
        <SummaryMetric label="Expiry Date" value={method.expiry} />
        <SummaryMetric label="Card Type" value={method.type} />
      </div>
      <div className="flex flex-wrap justify-end gap-2">
        <button
          type="button"
          onClick={onEdit}
          className="h-8 rounded-md border border-border bg-card/60 px-2.5 text-xs font-medium hover:bg-card transition-colors duration-150"
        >
          Edit
        </button>
        {onSetDefault && (
        ...
        {onRemove && (
          <button
            type="button"
            onClick={onRemove}
            className="h-8 rounded-md border border-destructive/30 bg-destructive/10 px-2.5 text-xs font-medium text-destructive hover:bg-destructive/15 transition-colors duration-150"
          >
            Remove
          </button>
        )}
...
function PaymentMethodModal({
  ...
  const save = () => {
    ...
    onSave({
      ...
      masked: `•••• ${digits.slice(-4) || method?.masked.slice(-4) || "0000"}`,
      holder,
      expiry,
      type: "Credit Card",
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid h-8 w-8 place-items-center rounded-md hover:bg-gray-50 transition-colors duration-150"
            aria-label="Close"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        <div className="flex justify-end gap-2 border-t border-border px-5 py-4">
          ...
          <button
            ...
            className="h-9 rounded-md bg-primary px-3 text-[14px] font-medium text-primary-foreground hover:bg-primary/90 transition-colors duration-150"
          >
            {method ? "Save Changes" : "Save Payment Method"}
          </button>
        </div>
      </div>
    </div>
...
function EmptyState({ title, description }: { title: string; description: string }) {
  return (
    <div className="grid place-items-center px-5 py-10 text-center">
      <ReceiptText className="mb-3 h-8 w-8 text-muted-foreground/50" />
      <h4 className="m-0 text-sm font-medium">{title}</h4>
      <p className="m-0 mt-1 text-xs text-muted-foreground">{description}</p>
    </div>
  );
...
const addonInvoices: InvoiceRow[] = [
  ...
  },
  {
    ...
    dateKey: "jun-2026",
    amount: "Rp299,000",
    description: "Additional 5,000 Contacts",
    status: "Paid",
  },
];
...
const plans = [
  ...
  {
    name: "Pro",
    priceLabel: "IDR 2,390,000/month",
    numericPrice: 2390000,
    button: "Switch Plan",
    features: [
      "10 users",
      "5,000 contacts",
  ...
  {
    name: "Premium",
    priceLabel: "IDR 4,190,000/month",
    numericPrice: 4190000,
    badge: "Popular",
    button: "Switch Plan",
    features: [
      "20 users",
...
const initialPaymentMethods: PaymentMethod[] = [
  {
    ...
    expiry: "07/28",
    type: "Credit Card",
    billingAddress: "Jl. Sudirman No. 18",
    country: "Indonesia",
    isDefault: true,
  },
  {
