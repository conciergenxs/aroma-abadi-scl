...
import { templates as allTemplates, type Template } from "@/components/scl/mock-data";
...
export function TemplatePicker({ open, onClose, onInsert }: Props) {
  ...
  const [starred, setStarred] = useState<Set<string>>(new Set(["tp2", "tp4"]));
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown" || e.key === "ArrowUp") {
        ...
        e.preventDefault();
        const idx = Math.max(0, filtered.findIndex((t) => t.id === selected?.id));
        const next = e.key === "ArrowDown" ? (idx + 1) % filtered.length : (idx - 1 + filtered.length) % filtered.length;
  ...
  }, [open, filtered, selected, onClose, onInsert]);
