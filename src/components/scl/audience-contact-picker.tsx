...
function BrandChips({ brands }: { brands: string[] }) {
  ...
  if (brands.length === 0)
    return <span className="text-[10px] text-muted-foreground italic">No purchases yet</span>;
  ...
  return (
    <div className="flex flex-wrap items-center gap-1">
      {shown.map((b) => (
        <span
          key={b}
          className="inline-flex items-center rounded-full border border-border bg-card px-1.5 py-0.5 text-[10px] text-foreground"
        >
          {b}
        </span>
      ))}
        ...
        <div className="rounded-lg border border-border overflow-hidden">
          ...
          <div className="min-h-[360px]">
            {pagedEligible.length === 0 ? (
              <p className="px-3 py-12 text-[13px] text-muted-foreground text-center italic">
              ...
              <div className="divide-y divide-border/60">
                {pagedEligible.map((c) => {
                  ...
                  return (
                    <label
                      key={c.id}
                      className={`flex items-start gap-2.5 px-3 py-2.5 cursor-pointer hover:bg-muted transition-colors ${checked ? "bg-primary/8" : ""}`}
                    >
                      <input
                        type="checkbox"
                        checked={checked}
                      ...
                      />
                      <span className="h-7 w-7 rounded-full bg-muted border border-border grid place-items-center text-[10px] font-semibold shrink-0">
                        {c.avatar}
                      </span>
                        ...
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="text-right shrink-0">
                            ...
                            <div className="text-[10px] text-muted-foreground">
                              Avg {formatIDR(stats.avgMonthlySpend)}/mo
                            </div>
                          </div>
                        </div>
                ...
                {pagedStaged.map((c) => (
                  <div key={c.id} className="flex items-center gap-2 px-3 py-2">
                    ...
                    <button
                      ...
                      className="h-5 w-5 shrink-0 grid place-items-center rounded-full text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors duration-150"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </div>
